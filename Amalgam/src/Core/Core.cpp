...
static inline bool CheckDXLevel()
{
	...
	{
		//const char* sMessage = "You are running with graphics options that Korone does not support. -dxlevel must be at least 90.";
		const char* sMessage = "You are running with graphics options that Korone does not support. It is recommended for -dxlevel to be at least 90.";
		U::Core.AppendFailText(sMessage);
		SDK::Output("Korone", sMessage, { 175, 150, 255 }, OUTPUT_CONSOLE | OUTPUT_DEBUG | OUTPUT_TOAST | OUTPUT_MENU);
		//return false;
	}
...
void CCore::LogFailText()
{
	...
	try
	{
		...
		file.close();
		m_ssFailStream << "Logged to Korone\\fail_log.txt. ";
	}
	catch (...) {}
...
void CCore::Unload()
{
	...
	SDK::Output("Korone", "Unloaded", { 175, 150, 255 }, OUTPUT_CONSOLE | OUTPUT_DEBUG);
}