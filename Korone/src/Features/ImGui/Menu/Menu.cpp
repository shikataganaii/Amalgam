...
void CMenu::DrawMenu()
{
	...
	if (Begin("Main", nullptr, ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoBackground))
	{
		...
		else if (!Vars::Menu::CheatTitle.Value.empty()) // title
		{
			...
			SetCursorPos({ H::Draw.Scale(12), H::Draw.Scale(11) });
			PushStyleColor(ImGuiCol_Text, F::Render.Accent.Value);
			FText(TruncateText(Vars::Menu::CheatTitle.Value, H::Draw.Scale(flSideSize - 28), F::Render.FontTitle).c_str(), FTextEnum::None, F::Render.FontTitle);
			PopStyleColor();
		}
		...
		FTabs(
			{
				...
				{ "Movement", "Main" },
				{ "Misc", "Main", "HvH" },
				{ "Logs", "Player List", "Settings##", "Output" },
				{ "Settings", "Config", "Binds", "Materials", "Extra" }
			},
			{ &iTab, &iAimbotTab, &iVisualsTab, &iMovementTab, &iMiscTab, &iLogsTab, &iSettingsTab },
		...
		if (BeginTable("MovementTable", 1))
		{
			...
			TableNextColumn();
			{
				if (Section("Movement"))
				{
					...
					FToggle(Vars::Misc::Movement::AutoJumpbug, FToggleEnum::Right); // this is unreliable without setups, do not depend on it!
			...
			PushDisabled(F::PlayerUtils.m_bLoad);
			{
				...
				if (FButton("Export", FButtonEnum::Fit | FButtonEnum::SameLine))
				{
					...
					if (fStream.is_open())
					{
						...
						SDK::Output("Korone", "Copied playerlist to clipboard", { 175, 150, 255 }, OUTPUT_CONSOLE | OUTPUT_DEBUG | OUTPUT_TOAST | OUTPUT_MENU);
					}
				}

				{
					...
					if (FButton("Import", FButtonEnum::Fit | FButtonEnum::SameLine))
					{
						...
						{
							SDK::Output("Korone", "Failed to import playerlist", { 175, 150, 255, 127 }, OUTPUT_CONSOLE | OUTPUT_DEBUG | OUTPUT_TOAST | OUTPUT_MENU);
						}
					}
					...
					if (FBeginPopupModal("Import playerlist", nullptr, ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_AlwaysUseWindowPadding))
					{
						...
						if (FButton("Import", FButtonEnum::Left))
						{
							...
							F::PlayerUtils.m_bSave = true;
							SDK::Output("Korone", "Imported playerlist", { 175, 150, 255 }, OUTPUT_CONSOLE | OUTPUT_DEBUG | OUTPUT_TOAST | OUTPUT_MENU);
				...
				if (FButton("Backup", FButtonEnum::Fit | FButtonEnum::SameLine))
				{
					try
					{
						...
						SDK::Output("Korone", "Saved backup playerlist", { 175, 150, 255 }, OUTPUT_CONSOLE | OUTPUT_DEBUG | OUTPUT_TOAST | OUTPUT_MENU);
					}
					catch (...)
					{
						SDK::Output("Korone", "Failed to backup playerlist", { 175, 150, 255, 127 }, OUTPUT_CONSOLE | OUTPUT_DEBUG | OUTPUT_TOAST | OUTPUT_MENU);
					}
				}
			}
			PopDisabled();
...
void CMenu::Render()
{
	...
	if (m_bIsOpen)
	{
		...
		AddDraggable("Velocity", Vars::Menu::VelocityDisplay, FGet(Vars::Menu::Indicators) & Vars::Menu::IndicatorsEnum::Velocity);
